...
class GoodService extends Service {
  ...
  checkGoodName(info) {
    ...
    return new Promise(function(resolve, reject) {
      try {
        const good = ctx.service.dao.index.findOne('GoodModel', {
  ...
  createGoodInfo(info) {
    ...
    return new Promise(async function(resolve, reject) {
      try {
        const newGood = await ctx.service.dao.index.create('GoodModel', _.clone(info));
        newGood.goods_cat =
  ...
  //     try {
  //       const oldpics = ctx.service.dao.index.list('GoodPicModel', {
  ...
  //         // 1.1 删除图片物理路径
  //         batchFns.push(this.removeGoodPicFile(path.join(process.cwd(), pic.pics_big)));
  //         batchFns.push(this.removeGoodPicFile(path.join(process.cwd(), pic.pics_mid)));
  //         batchFns.push(this.removeGoodPicFile(path.join(process.cwd(), pic.pics_sma)));
  //         // 1.2 数据库中删除图片数据记录
